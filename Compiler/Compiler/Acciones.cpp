...
bool Expandir::sePuedeAplicar(Estado_Compilador* stte) {
	...
	for (i = 0; i < chart.size(); ++i) {
		if (chart[i]->producRef->nombre.name == stte->producRef->nombre.name) return false;
	}
	return true;
}
...
Aceptar::Aceptar(Estado_Compilador* state) {
	if (sePuedeAplicar(state)) {
		aplica(state);
	}
}
...
Unificar::Unificar(Estado_Compilador* state) {
	if (sePuedeAplicar(state)) {
		aplica(state);
	}
}
